...
Canonical role content is in `roles/`; metadata and model routing are in `catalog.json`; canonical skills are in `skills/`. Change those sources, then run `python scripts/sync_agents.py` from this repository or `agent-loom sync` when the package is installed. Never hand-edit generated native roles or mirrored Claude skills.
